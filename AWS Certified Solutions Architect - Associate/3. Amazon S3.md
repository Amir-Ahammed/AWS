...
Lifecycle policies in Amazon S3 help you **automate the management of objects** over time, reducing costs and improving efficiency.

### Purpose
- Automatically **transition objects** to cheaper storage classes.
- **Expire (delete)** objects after a set period.
- Apply rules to **current and previous versions** (if versioning is enabled).

### Key Components

| Component         | Description                                                                 |
|------------------|-----------------------------------------------------------------------------|
| **Transition**    | Move objects to another storage class (e.g., Standard-IA, Glacier).         |
| **Expiration**    | Permanently delete objects after a specified time.                          |
| **Filter**        | Apply rules to specific objects using **prefixes** or **tags**.             |
| **Versioning**    | Separate rules for **current** and **noncurrent** object versions.          |


### Example Use Cases

- Transition logs to **S3 Glacier** after 90 days.
- Delete temporary files **30 days** after creation.
- Move infrequently accessed backups to **S3 Intelligent-Tiering**.
