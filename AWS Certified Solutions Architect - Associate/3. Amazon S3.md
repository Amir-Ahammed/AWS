...
> 📌 *S3's flexibility makes it a foundational component for cloud-native architectures across many industries.*
...
- **IAM permissions** must allow S3 to replicate objects on your behalf.
- Replication is **asynchronous** — there's a small delay between source upload and destination availability.
- Buckets can belong to the **same or different AWS accounts**.

### Key Characteristics
- You can choose to replicate **all objects** or only those with specific prefixes or tags.
- Supports **replication of delete markers**, **ACLs**, and **object metadata**.
...
| Aggregating logs across buckets         | SRR                          |
| Live sync between prod and dev buckets  | SRR                          |
| Backup to separate AWS account          | CRR or SRR                  |
